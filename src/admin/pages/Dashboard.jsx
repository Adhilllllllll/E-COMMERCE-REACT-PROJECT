...
const DashboardContent = () => {
  const { users, loading: usersLoading, error: usersError } = useContext(UserContext);
  ...
  const activeAccounts = users.filter((u) => !u.isBlocked && !u.isDelete).length;
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        !user.isBlocked && !user.isDelete ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                      }`}
                    >
                      {!user.isBlocked && !user.isDelete ? "Active" : "Inactive"}
                    </span>
                  </td>
                </tr>
              ))}
