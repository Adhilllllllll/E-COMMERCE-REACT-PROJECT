...
const ProductManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingProduct) {
      await editProduct(editingProduct.id, normalizedData, selectedFiles);
    } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <table className="min-w-full divide-y divide-gray-200 mb-6">
        <thead>
          <tr>
            {["Images", "Name", "Category", "Price", "Stock", "Status", "Actions"].map((col) => (
          ...
          {products.map((p) => (
            <tr key={p.id}>
              <td className="px-4 py-2 flex gap-2">
                {p.images?.map((img, i) => <img key={i} src={img} className="w-12 h-12 object-cover" />)}
              </td>
              <td>{p.name}</td>
              <td>{p.category}</td>
              <td>${p.price}</td>
              ...
              <td>{p.status}</td>
              <td>
                <button onClick={() => openForm(p)} className="text-blue-600">Edit</button>
                <button onClick={() => {
                  Swal.fire({
                    title: "Are you sure?",
                    text: "You won't be able to revert this!",
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <input type="text" placeholder="Category" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full border p-2 rounded" required />
              ...
              <input type="number" placeholder="Stock" value={formData.count} onChange={(e) => setFormData({...formData, count: e.target.value})} className="w-full border p-2 rounded" required />
              <label>Images</label>
              <input type="file" multiple onChange={handleFileChange} />
              <label>Status</label>
              <select value={formData.isActive} onChange={(e) => setFormData({...formData, isActive: e.target.value === "true"})} className="w-full border p-2 rounded">
              ...
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">{editingProduct ? "Update" : "Add"}</button>
              </div>
            </form>
          </div>
        </div>
