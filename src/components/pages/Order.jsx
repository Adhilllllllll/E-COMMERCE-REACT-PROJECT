...
const Order = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      // if there’s no orderId, redirect to orders page
      if (!orderId) {
        setLoading(false);
        navigate("/orders");
        return;
      ...
      try {
        // ✅ If we already have the order (navigated from OrderHistory)
        if (location.state?.order) {
          setOrder(location.state.order);
        } else {
          // 🔄 If user refreshed, fetch from backend
    ...
    fetchOrder();
  }, [orderId, location.state?.order, navigate]);
